...
def configure(conf):
  ...
  conf.check(lib='png', libpath=['/lib', '/usr/lib', '/usr/local/lib', '/usr/local/libpng/lib', '/usr/X11/lib', '/usr/local/pkg/libpng-1.4.1/lib'])
...
def build(bld):
  ...
  obj.uselib = "PNG"
  obj.cxxflags = ["-D_FILE_OFFSET_BITS=64", "-D_LARGEFILE_SOURCE", "-I/usr/X11/include"]
